...
function _triggerZoneEvent(zone, motion) {
  ...
  if (alarm.active && alarm.zones[zone].active && !alarm.triggered) {
...
async function _pingBuzzer() {
  if (alarm.arming) {
    await _toggleOutput(
      "buzzers",
      true,
      alarmConfig.buzzer.beepDuration,
      false
    );
    await _pauseBuzzer();
  }
...
async function _countDownBuzzer() {
  const counter =
    alarmConfig.armDuration /
    (alarmConfig.buzzer.beepDuration = alarmConfig.buzzer.beepPause);
